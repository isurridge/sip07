    ...
    var selModel = Ext.create('Ext.selection.CheckboxModel', {
        listeners: {
            selectionchange: function(sm, selections) {
            	//Ext.getCmp('grid7').down('#removeButton').setDisabled(selections.length == 0);
            }
        }
    });
	...
	return {
		...
		tabsNestedLayouts : {
			...
			items : [ {
				...
				// A common mistake when adding grids to a layout is creating a
				// panel first,
				// then adding the grid to it. GridPanel (xtype:'grid') is a
				// Panel subclass,
				// so you can add it directly as an item into a container.
				// Typically you will
				// want to specify layout:'fit' on GridPanels so that they'll
				// size along with
